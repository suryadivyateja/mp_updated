...
extras=[];
  ngOnInit() {
    ...
    this.gigService.get_orderby_id(this.order_id).subscribe(order => {
      ...
      this.attachment = or.resume;
      for(var key in JSON.parse(or.selected_extras)) {
        var value = JSON.parse(or.selected_extras)[key];
        this.extras.push(value);
    }
    console.log(this.extras);
      this.gigService.get_gig_byId(this.gig_id).subscribe(gig => {
            // console.log(gig);
             this.img = gig.msg.img1,
